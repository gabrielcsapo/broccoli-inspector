    ...
    Query: {
      info(root, args, context, info) {
        return new Promise(function(resolve, reject) {
          const broccoliPath = require.resolve('broccoli');
          const packagePath = findUp.sync('package.json', { cwd: path.dirname(broccoliPath) });

          // this env variable is used in testing to ensure we can trigger this flow
          if(semver.satisfies(require(packagePath).version, '>= 3') && !process.env.BROCCOLI_INSPECTOR_NOT_SUPPORTED) {
            return resolve({})
          } else {
            return resolve({
              notSupported: {
                reason: 'We currently only support projects using Broccoli > 3',
              }
            })
          }
        });
