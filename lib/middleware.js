...
module.exports = function (app, info) {
  ...
  const nodeWrappersByIdLookup = watcher.builder
    ? watcher.builder.builder._nodeWrappers
      ? nodeWrappersById(watcher.builder.builder._nodeWrappers)
      : {}
    : {};
  ...
  if (watcher.builder) {
    ...
    io.on("connection", function (socket) {
      ...
      watcher.on("buildFailure", (node) => {
        node.broccoliPayload.fileContents = fs.readFileSync(
          path.resolve(
            node.broccoliPayload.location.treeDir,
            node.broccoliPayload.location.file
          ),
          "utf8"
        );
  ...
  function nodeWrappersById(nodeWrappers) {
    ...
    for (const [, _node] of nodeWrappers.entries()) {
    ...
    node.inputNodeWrappers.forEach((inputNodeWrapper) => {
      if (parseInt(inputNodeWrapper.id) === parseInt(id)) {
        ...
        if (inputNodeWrapper.inputNodeWrappers) {
          outputNodeWrappers = outputNodeWrappers.concat(
            findOutputNodeWrappers(inputNodeWrapper, id)
          );
        }
      }
    });
  ...
  const resolvers = {
    Query: {
      info(root, args, context, info) {
        ...
        if (
          !dep.satisfies(">= 3.11.0") &&
          !process.env.BROCCOLI_INSPECTOR_NOT_SUPPORTED
        ) {
          return {
            notSupported: {
              reason:
                "We currently only support projects using ember-cli > 3.11.0",
            },
          };
        }
        ...
        if (!watcher.builder) {
          return {
            notSupported: {
              reason:
                "There is no builder available, please serve your ember app with `ember serve` without a dist specified to retrieve a builder.",
            },
          };
        }
      ...
      nodesByType(root, args, context, info) {
        ...
        for (const [
          ...
          const broccoliPluginName = cleanBroccoliPluginName(
            heimdallId.broccoliPluginName
          );
      ...
      dashboard() {
        return {
          ...
          selfTime: Array.from(
            ...
            .map((inputNode) => inputNode.buildState.selfTime)
            .reduce((a, b) => (a || 0) + (b || 0)),
    ...
    Node: {
      ...
      stats: (root, args, context, info) => {
        ...
        if (node["__heimdall__"]._children) {
          custom = node["__heimdall__"]._children
            .filter((c) => Object.keys(c.stats.own).length > 0)
            .map((c) => {
              return {
                name: c.id.name,
      ...
      outputNodeWrappers: (root, args, context, info) => {
        ...
        for (const [
          ...
          outputNodeWrappers = outputNodeWrappers.concat(
            findOutputNodeWrappers(_node, id)
          );
        }
      ...
      slowestNodes: (root, args, context, info) => {
        return Array.from(
          ...
          .sort(
            (pluginA, pluginB) =>
              (pluginB.buildState.selfTime || 0) -
              (pluginA.buildState.selfTime || 0)
          )
          .slice(0, 5);
      },
